...
<?php if(isset($_POST['editar'])){
	$texto1 = $_POST['texto1'];
	$atualiza = mysql_query("UPDATE clientes SET texto = '$texto1' ");
    if ($atualiza =='0'){
...
<strong>Edi&ccedil;&atilde;o da P&aacute;gina Clientes </strong>
...
<form id="form1" name="form1" method="post" action="" enctype="multipart/form-data">
  ...
  $id = $_GET['id'];
   $seleciona_texto = mysql_query("SELECT * FROM clientes WHERE id = '1'")or die("Erro ao selecionar as fotos".mysql_error());
   if($seleciona_texto == ''){
	   echo 'Nenhum Registro';
   }else{
	 while($res_id = mysql_fetch_array($seleciona_texto)){
	$clientes = $res_id['texto'];
		   
	 }
  ?>
    <textarea name="texto1" id="texto1" cols="45" rows="5"><?php echo $clientes;}?>
    </textarea>
    <p>
